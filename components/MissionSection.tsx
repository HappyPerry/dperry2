...
const defaultPillars: MissionPillar[] = [
  {
...
export default function MissionSection({ data }: MissionSectionProps) {
  const eyebrow = data?.eyebrow ?? "Our Mission";
  const headlineLine1 = data?.headlineLine1 ?? "Continuing What";
  const headlineLine2 = data?.headlineLine2 ?? "D'Sean Started";
  const missionQuote =
    data?.missionQuote ??
  ...
  const pillarsList = data?.pillars && data.pillars.length > 0 ? data.pillars : defaultPillars;
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 lg:mb-24">
          ...
          <ScrollReveal delay={0.1}>
            <h2 className="headline-lg text-4xl sm:text-5xl lg:text-6xl text-text-primary mb-8">
              {headlineLine1}
              <span className="block text-uva-blue mt-1">{headlineLine2}</span>
            </h2>
          </ScrollReveal>
